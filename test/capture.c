...
int main() {
  ...
  while (loops-- > 0) {
    ...
    if (frames == -EPIPE || frames == -ESTRPIPE) {
      ...
      fprintf(stderr, "Expected to read %ld frames from audio device but only \
read %ld\n",
              period_size_in_frames, frames);
      break;
    }
    ssize_t bytes = write(STDOUT_FILENO, buf, period_size_in_bytes);
    if (bytes < 0) {
      perror("Failed to write to stdout");
      break;
    } else if (bytes != period_size_in_bytes) {
      fprintf(stderr,
              "Expected to write %d bytes to stdout but only wrote %ld\n",
