...
void print_jb(jb_t *jb) {
  ...
  while (entry != NULL) {
    fprintf(stderr, "%ld:%s -> ", entry->timestamp, entry->data);
    entry = entry->next;
  }
  fprintf(stderr, "\n");
  /*
  entry = jb->head;
  while (entry != NULL) {
    fprintf(stderr, "%d:%s <- ", entry->timestamp, entry->data);
